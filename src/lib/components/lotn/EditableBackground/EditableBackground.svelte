<script lang="ts">
	...
	function hasBackgroundBeenPaidWithDots(id: string) {
		const store = get(backgroundPaymentStore.paymentStore);
		const result = store.backgrounds.some((background) => background.id === id);
...
<div id={background.id} class="flex flex-col gap-2">
	<div class="flex h-10">
		<h4 class="h4">{background.name}</h4>

		{#if !isFixedBackground(background.id) && !hasBackgroundBeenPaidWithDots(background.id) && editModeEnabledAdvantages}
			<button
